---
title: 'R-code for "Adult sex ratio bias is driven by sex-specific early survival: implications for population growth and mating system evolution"'
...
h <- 
  as.numeric(sex_specific_mating_system[which(sex_specific_mating_system$sex == "Female"), 2])
# display the h value
h
```
...
```
Figure 1b: plot the sex-differences in mating behaviour
...
Background_LTRE_ASR <-
  ...
  theme(text = element_text(#family="Arial", 
        ...
        # plot.margin = unit(c(1,0.5,0.5,2.45), "cm"),
        panel.margin = unit(0.75, "lines"),
        strip.background = element_blank(),
        strip.text = element_blank()) +
...
# Find the model number for the first ranked model of the AIC table
model_chick_num <- as.numeric(rownames(chick_survival_year_sex_age_run$model.table[8,]))
...
chick_survival_year_age = function() 
{
  ...
  Phi.quadratic_year = list(formula = ~ quadratic + year)
...
survival_rates_year <- list(survival_rates_year_age = survival_rates_year_age,
                            survival_rates_year_age_sex = survival_rates_year_age_sex)
survival_rates_year
}
# uncomment this to run the annual survival analysis
#survival_rates_year <- annual_survival_analysis()
```
...
```{r}
survival_rates_year_age <- read.table(file = "~/Dropbox/Luke/R_projects/Ceuta_ASR_Matrix_Modeling/output/annual_estimates/survival_rates_year_age.txt", header = TRUE)
survival_rates_year_age_sex <- read.table(file = "~/Dropbox/Luke/R_projects/Ceuta_ASR_Matrix_Modeling/output/annual_estimates/survival_rates_year_age_sex.txt", header = TRUE)
```
consolidate the year-specific vital rates into a list (note: the annual fecundity estimates were obtained in the sex-sepecific fecundity analysis at the beginning of this document)
```{r}
...
two_sex_stoch_proj_year <- stoch_projection_plover(matrices = year_mats, n0 = n_two_sex, h = h,
                                            tmax = 7, nreps = 10000, Quasi_Ex = 2, HSR = HSR)

one_sex_stoch_proj_year <- stoch_projection_plover(matrices = year_mats, n0 = n_one_sex, 
                                                   tmax = 7, nreps = 10000, Quasi_Ex = 2, HSR = 0.5, no_stages = 2, two_sex = FALSE)
```
...
```
Figure 3a: plot the actual vs. the predicted lambda distributions of the two-sex and one-sex model simulations between 2006 and 2012.
...
ggplot() +
  ...
  geom_errorbarh(data = two_sex_stoch_proj_year_summary,
                 aes(y = 975, x = lcl, xmin = lcl, xmax = ucl),
                 color = "black", size = 0.8, linetype = "solid") +
  geom_errorbarh(data = one_sex_stoch_proj_year_summary,
                 aes(y = 950, x = lcl, xmin = lcl, xmax = ucl), 
  ...
  # xlab(expression('Stochastic population growth (\u03BB'['S']*')')) +
...
lambda_elasticities$model_vital_rate <- factor(lambda_elasticities$model_vital_rate,
                                               levels = c("equal_mono_Adult_survival", "equal_mono_Fledgling_survival", "equal_mono_Chick_survival",
                                                          ...
                                                          "diff_poly_Adult_survival", "diff_poly_Fledgling_survival", "diff_poly_Chick_survival",
                                                          "diff_poly_Hatching_sex_ratio", "diff_poly_Mating_system"))
```
Figure 3b: plot the elasticities of lambda to perturbations of each parameter
...
Background_lambda_elasticity <-
  ...
  annotate("rect", xmin=5.02, xmax=9.98, ymin=-Inf, ymax=Inf, alpha=0.6,
           fill=custom_pal[3]) +
  annotate("rect", xmin=9.98, xmax=14.93, ymin=-Inf, ymax=Inf, alpha=0.6,
           fill=custom_pal[2]) +
  annotate("rect", xmin=14.93, xmax=20, ymin=-Inf, ymax=Inf, alpha=0.6,
           fill=custom_pal[1]) +
  ...
  theme(text = element_text(#family="Arial", 
        ...
        axis.ticks.length = unit(0.2, "cm"),
        panel.border = element_blank(),
        plot.margin = unit(c(0.1,0.5,-0.4,2.1), "cm"),
        # plot.margin = unit(c(0.2,0.5,-0.3,1.7), "cm"),
        panel.margin = unit(0.75, "lines"),
        strip.background = element_blank(),
        strip.text = element_blank()) +
...
lambda_elasticity <- 
  ggplot2::ggplot() +
          ...
          scale_fill_manual(values = cbPalette) +
          scale_y_continuous(limits = c(-0.5, 0.5)) +
          # scale_x_discrete(labels = c("equal_mono_Adult_survival" = expression(Adult["\u03D5"]),
          ...
          #                             "diff_poly_Adult_survival" = expression(Adult["\u03D5"]),
          ...
          scale_x_discrete(labels = c( "equal_mono_Adult_survival" = "Adult surv",
                                      ...
                                      "diff_poly_Adult_survival" = "Adult surv",
