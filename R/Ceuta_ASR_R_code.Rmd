...
# because the mating system is polyandrous, h must be less than 1, and therefore the inverse is calculated
h <- 
...
h
```
plot the sex-specific distributions of mating system
...
cbPalette <- RColorBrewer::brewer.pal(8, "Dark2")[c(1,2)]
    ...
    if(two_sex){
      ...
      result <- 
        matrix(c(
                 ...
                 0, RF,
                 # second row of matrix
                 (demographic_rates$Chk_survl*demographic_rates$Fdg_survl),
...
this function extracts the ASR and lambda estimates from each bootstrap iteration using a matrix with the two-sex mating function
```{r}
lambda_extract <-
  function(survival_rates, niter = 1000){
    ...
    for(i in 1:niter){
      ...
      VR_one_sex_boot <- list(Chk_survl = mean(survival_rates[which(survival_rates$iter == i), 2][6],
                              ...
                              Fdg_survl = mean(survival_rates[which(survival_rates$iter == i), 2][4],
      ...
      # Determine the ASR at the stable stage distribution
      two_sex_lambda <-
        matrix_ASR(M = two_sex_matrix_boot, h = VR_two_sex_boot$h, HSR = VR_two_sex_boot$HSR, iterations = 75)
      one_sex_lambda <-
...
```
calculate the 95% confidence interval, mean, and median of the ASR bootstraps
```{r}
CI <- 0.95
lambda_boot_summary <-
  lambda_boot%>%
...
```
Store the actual annual population size estimate from the field and calculate the geometric mean of the popualtion growth rate observed over the seven-year period
```{r}
...
Actual_lambda_geom_mean <- gm_mean(Actual_lambda)
```
...
ggplot() +
  ...
  geom_histogram(binwidth = 0.005, data = filter(lambda_boot, parameter == "one_sex_lambda"),
                 aes(x = estimate, fill = parameter), alpha = 0.7) +
  geom_vline(xintercept = Actual_lambda_geom_mean, color = brewer.pal(8, "Set1")[c(2)], size=1) +
